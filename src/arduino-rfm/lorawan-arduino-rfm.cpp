...
bool LoRaWANClass::init(void) {
    ...
    LoRa_Settings.Channel_Hopping =
        0x00;  // 0x00 no channel hopping, 0x01 channel hopping
...
bool LoRaWANClass::join(void) {
    ...
    prev_millis = millis();
    do {
        join_status = LORA_join_Accept(&Buffer_Rx, &Session_Data, &OTAA_Data,
                                       &Message_Rx, &LoRa_Settings);
...
void LoRaWANClass::wakeUp(void) { RFM_Switch_Mode(RFM_MODE_STANDBY); }
...
void LoRaWANClass::sendUplink(char *data, unsigned int len,
                              unsigned char confirm, unsigned char mport) {
    if (currentChannel == MULTI) {
        randomChannel();
    }
...
unsigned char LoRaWANClass::getChannel() { return LoRa_Settings.Channel_Tx; }

unsigned char LoRaWANClass::getDataRate() { return LoRa_Settings.Datarate_Tx; }
...
void LoRaWANClass::update(void) {
    ...
    if ((RFM_Command_Status == NEW_RFM_COMMAND || RFM_Command_Status == JOIN) &&
        ...
        // LoRaWAN TX/RX cycle
        LORA_Cycle(&Buffer_Tx, &Buffer_Rx, &RFM_Command_Status, &Session_Data,
                   &OTAA_Data, &Message_Rx, &LoRa_Settings);

        if ((Message_Rx.Frame_Control & 0x20) > 0) Ack_Status = NEW_ACK;
    ...
    if (LoRa_Settings.Mote_Class == CLASS_C) {
        ...
        if (RFM_Command_Status == NEW_RFM_COMMAND) {
            // LoRaWAN TX/RX cycle
            LORA_Cycle(&Buffer_Tx, &Buffer_Rx, &RFM_Command_Status,
                       &Session_Data, &OTAA_Data, &Message_Rx, &LoRa_Settings);
            if (Buffer_Rx.Counter != 0x00) {
                Rx_Status = NEW_RX;
            }
            RFM_Command_Status = NO_RFM_COMMAND;
        ...
        if (digitalRead(RFM_pins.DIO0) == HIGH) {
            LORA_Receive_Data(&Buffer_Rx, &Session_Data, &OTAA_Data,
                              &Message_Rx, &LoRa_Settings);
            if (Buffer_Rx.Counter != 0x00) {
                Rx_Status = NEW_RX;
            }
...
unsigned int LoRaWANClass::getFrameCounter() { return Frame_Counter_Tx; }
